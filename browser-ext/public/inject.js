(function() {
  ...
  function checkInventory() {
    try {
      if (typeof UserYou === 'undefined' || !UserYou.getInventory) return false;
      if (!UserYou.rgAppInfo || !UserYou.rgAppInfo[730]) return false;
      var inv = null;
      try { inv = UserYou.getInventory(730, 2); } catch(ignore) { return false; }
      if (!inv || !inv.m_rgAssets || Object.keys(inv.m_rgAssets).length === 0) return false;

      var items = [];
      var allProps = inv.m_rgAssetProperties || {};

      for (var key in inv.m_rgAssets) {
        var asset = inv.m_rgAssets[key];
        if (asset && asset.description) {
          var item = {
            assetid: asset.assetid,
            name: asset.description.market_hash_name || asset.description.name,
            type: asset.description.type,
            rarity_color: (asset.description.tags || []).find(function(t) { return t.category === 'Rarity'; }),
            float: null,
            paintSeed: null,
            paintIndex: null
          };
          if (item.rarity_color) item.rarity_color = item.rarity_color.color;

          var props = allProps[asset.assetid];
