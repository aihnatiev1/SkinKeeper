...
-- 028: Steam item_nameid cache for histogram API
...
-- 029: Per-account daily P/L snapshots — replace user-only unique constraint
-- Drop constraint first (it owns the index), then create new composite one
...
-- 034: Auto-sell rules & executions (P3 premium feature).
...
CREATE TABLE IF NOT EXISTS auto_sell_rules (
  id                    SERIAL PRIMARY KEY,
  user_id               INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  -- Account scope: rule fires only for items owned by this Steam account.
  -- Keep NOT NULL — widening to user-wide later is non-breaking.
...
-- Cron hot-path: "give me all enabled rules" (partial keeps it small).
...
-- Cross-rule fan-out by item name.
...
CREATE TABLE IF NOT EXISTS auto_sell_executions (
  ...
  -- 5 valid actions: 'notified', 'listed', 'cancelled', 'failed', 'pending_window'
...
-- Used by the 60s cancel-window worker to find pending rows.
...
-- FK index for joins back to sell_operations (avoids seq scan on cascade).
...
-- 035: User feature flags
-- JSONB column for per-user feature flag overrides. Merged at request time
-- with env-driven kill switches and canary rollout computation by
