...
// POST /api/auth/token — Login via steamLoginSecure
router.post("/token", async (req: Request, res: Response) => {
  try {
    ...
    const steamId = await SteamSessionService.extractSteamIdFromCookie(steamLoginSecure);
    if (!steamId) {
    ...
    // Extract session ID and save session
    const sessionId = await SteamSessionService.extractSessionId(steamLoginSecure);
    if (sessionId) {
      await SteamSessionService.saveSession(accountId, { sessionId, steamLoginSecure });
      await pool.query(
        `UPDATE steam_accounts SET session_method = 'clienttoken', session_updated_at = NOW() WHERE id = $1`,
        [accountId]
      );
    }
